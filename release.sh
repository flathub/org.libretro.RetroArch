...
Options:
    ...
    --push        Push changes to remote git repository at the end
...
check_dependencies cat date git grep sed wget
read_arguments "$@"
validate_arguments
...
render_tmpl() {
    local tmpl_file="$1"
    local RA_VERSION_TAG="$(sed_replacement_escape "$(json_string_escape "$2")")"
    log_debug "render_tmpl() escaped RA_VERSION_TAG=$RA_VERSION_TAG"
    local result="$(cat "$tmpl_file")"
    local result="$(printf '%s' "$result" | sed -E "s/%%RA_VERSION_TAG%%/$RA_VERSION_TAG/g")"

    for repo in retroarch-assets libretro-database libretro-core-info retroarch-joypad-autoconfig \
                common-shaders slang-shaders glsl-shaders common-overlays; do
        local commit_json="$(wget -q -O- --header='accept: application/json' \
            "https://api.github.com/repos/libretro/$repo/commits?per_page=1&page=1")"
        local commit_sha="$(printf '%s' "$commit_json" | grep -E -o -m 1 '"sha"\s*:\s*"[0-9a-fA-F]{40}"')"
        local commit_sha="${commit_sha%\"}"
        local commit_sha="${commit_sha##*\"}"
        [ "$commit_sha" ] || abort "render_tmpl() null commit_sha for repo $repo. May have reached GitHub's rate limit. Try again in one hour"
        local commit_sha="$(sed_replacement_escape "$(json_string_escape "$commit_sha")")"
        log_debug "render_tmpl() commit_sha=$commit_sha repo=$repo"
        local result="$(printf '%s' "$result" | sed -E "s/%%${repo}_GIT_REF%%/$commit_sha/g")"
    done
...
if [ "$is_dry_run" ]; then
    ...
    printf '%s\n' "$render_tmpl_result" > "$MANIFEST_FILE" || abort "Failed to write file $MANIFEST_FILE"
fi
...
if [ "$is_dry_run" ]; then
    ...
    printf '%s\n' "$render_appstream_result" > "$APPSTREAM_FILE" || abort "Failed to write file $APPSTREAM_FILE"
fi
